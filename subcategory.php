...
if (isset($_GET['id'])) {
    $subcatName = mysqli_real_escape_string($conn, $_GET['id']);

    $sub_query = "SELECT * FROM subcategory WHERE id = '$subcatName' AND status = 1";
    ...
    if ($sub_result->num_rows > 0) {
        ...
        echo "<h4 class='text-center pt-3 pb-5'>" . htmlspecialchars($sub_row['name']) . "</h4>";

        $property_query = "SELECT * FROM property WHERE subcategory = '$subcatName' ORDER BY id DESC";
                            ...
                            <a href="property-details.php?id=' . $prop['id'] . '" class="images-group">
                                ...
                                <div class="top">
                                    <ul class="d-flex gap-6">
                                        <li class="flag-tag style-1">' . htmlspecialchars($categoryName) . '</li>
                                    </ul>
                                </div>
        ...
        echo "<p>No sub-category selected.</p>";
    }
}
